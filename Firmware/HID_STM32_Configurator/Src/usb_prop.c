...
uint8_t ReadBufer[wMaxPacketSize];
...
* Return         : None.
*******************************************************************************/
void flash_write(uint32_t StartAddress)
{
            //-----------Make Mass in uint8_t to uint16_t type----------------------------------------------------
uint16_t* ptr;
ptr=(uint16_t*)StartAddress;
int k=2;
int s=0;
int i=0;
uint16_t data[wMaxPacketSize/2];
    for(s=0;s<(wMaxPacketSize/2);s++)
    {
        data[s]=(uint16_t)(Report_Buf[k]|(Report_Buf[k+1]<<8));
        k+=2;
    }
            //-------------Flash Write-------------------------------------------------------------------
    FLASH_Unlock();
    FLASH->CR |= FLASH_CR_PG;                               // Sanction Flash Program

   for(i=0;i<(wMaxPacketSize/2);i++)
   {
        *(uint16_t*)ptr=(uint16_t)data[i];
        ptr++;
   }
   FLASH->CR &= ~(FLASH_CR_PG);                             // DeSanction Flash Program
   FLASH_Lock();
}
...
void HID_Status_In(void)
{
  //BitAction Led_State;
   switch (Report_Buf[1])
   {
    case 0xAA:
    __disable_irq();
    flash_write(ADDRESS_MSG_SYSTEM);
    __enable_irq();
    ...
    case 0xA1:
    __disable_irq();
    flash_write(ADDRESS_MSG_1);
    __enable_irq();
    break;

    case 0xA2:
    __disable_irq();
    flash_write(ADDRESS_MSG_2);
    __enable_irq();
    break;

    case 0xA3:
    __disable_irq();
    flash_write(ADDRESS_MSG_3);
    __enable_irq();
    break;

    case 0xA4:
    __disable_irq();
    flash_write(ADDRESS_MSG_4);
    __enable_irq();
    break;

    case 0xA5:
    __disable_irq();
    flash_write(ADDRESS_MSG_5);
    __enable_irq();
    break;

    case 0xA6:
    __disable_irq();
    flash_write(ADDRESS_MSG_6);
    __enable_irq();
    break;

    case 0xA7:
    __disable_irq();
    flash_write(ADDRESS_MSG_7);
    __enable_irq();
    break;

    case 0xA8:
    __disable_irq();
    flash_write(ADDRESS_MSG_8);
    __enable_irq();
    break;
    ...
    __disable_irq();
    FLASH_Unlock();
    FLASH_ErasePage(ADDRESS_MSG_SYSTEM);
    FLASH_Lock();
    __enable_irq();
    break;
   }
...
* Return         : None.
*******************************************************************************/
/*void readFlash(uint32_t StartReadAddress)
{
    uint32_t hip;
    int i =0;
    for(i=1;i<wMaxPacketSize-4;i++)
    {
        ReadBufer[i]=(*(uint8_t*)StartReadAddress);
        StartReadAddress+=1;
  //  hip=*(uint32_t*)StartReadAddress;
    }
