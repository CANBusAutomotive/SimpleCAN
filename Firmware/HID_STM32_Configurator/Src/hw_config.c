...
*******************************************************************************/
extern uint8_t ReadBufer[wMaxPacketSize-4];

void readFlash (uint32_t StartAddress)
{
    int i=0;
    for(i=2;i<wMaxPacketSize-4;i++)
    {
      ReadBufer[i] = *(uint8_t*)StartAddress;
      StartAddress+=1;
    }
...
uint8_t RHIDCheckState(void)
{
    ...
    PrevXferComplete = 0;
    ReadBufer[0] = 0x04;
    readFlash(ADDRESS_MSG_SYSTEM);
    ReadBufer[10] = 0x44;
    /* Copy mouse position info in ENDP1 Tx Packet Memory Area*/
    USB_SIL_Write(EP1_IN, ReadBufer, RPT4_COUNT+1);
